...
export const updateClaudeApiKey = action({
	...
	handler: async (ctx, args): Promise<string> => {
		...
		const validationResult = await ctx.runAction(
			internal.infrastructure.auth.validateApiKeys.validateClaudeApiKey,
			{
				apiKey: args.apiKey
			}
		);
		...
		const encryptedKey: string = await ctx.runAction(internal.infrastructure.crypto.encryptApiKey, {
...
export const updateReadwiseApiKey = action({
	...
	handler: async (ctx, args): Promise<string> => {
		...
		// Validate the API key first
		const validationResult = await ctx.runAction(
			internal.infrastructure.auth.validateApiKeys.validateReadwiseApiKey,
			{
				apiKey: args.apiKey
			}
		);
