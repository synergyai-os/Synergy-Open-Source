	...
	handler: async () => {
		...
		try {
			const result = await resend.emails.send({
				...
				subject: 'Test Email from SynergyOS',
				html: `
					<h1>Hello from SynergyOS!</h1>
				...
				`,
				text: 'Hello from SynergyOS! This is a test email to verify your Resend setup is working correctly. If you receive this, everything is configured properly!'
			});
...
export const sendPasswordResetEmail = internalAction({
	...
	handler: async (ctx, args) => {
		// E2E test mode - skip actual email sending
		if (process.env.E2E_TEST_MODE === 'true') {
			console.log('📧 [E2E Mock] Password reset email suppressed:', {
				to: args.email,
			...
			return {
				success: true,
				emailId: `mock-password-reset-${args.email}-${Date.now()}`
			};
		}
...
export const sendCircleInviteEmail = internalAction({
	...
	handler: async (ctx, args) => {
		...
		const resend = new Resend(apiKey);
		const roleDisplay = args.role === 'admin' ? 'Admin' : 'Member';
