...
describe('Organizations Integration Tests', () => {
	...
	it('should create organization with auto-generated slug', async () => {
		...
		cleanupQueue.push({ orgId: result.organizationId });
	});
	...
	it('should accept organization invite and create membership', async () => {
		const t = convexTest(schema, modules);
		...
		const membership = await t.run(async (ctx) => {
			return await ctx.db
				...
				.withIndex('by_organization_user', (q) =>
					q.eq('organizationId', orgId).eq('userId', inviteeUserId)
				)
				.first();
		});
	...
	it('should decline organization invite without creating membership', async () => {
		const t = convexTest(schema, modules);
		const { userId: inviterUserId } = await createTestSession(t);
		const { userId: inviteeUserId } = await createTestSession(t);
		const orgId = await createTestOrganization(t, 'Test Org');
	...
	it.skip('should remove organization member with RBAC permission check', async () => {
		const t = convexTest(schema, modules);
	...
	it('should enforce user isolation - users cannot see other orgs', async () => {
		...
		const user1Orgs = await t.query(api.organizations.listOrganizations, {
			sessionId: session1
		});
		...
		const user2Orgs = await t.query(api.organizations.listOrganizations, {
			sessionId: session2
		});
	...
	it('should fail with invalid sessionId', async () => {
		...
		await expect(
			t.query(api.organizations.listOrganizations, {
