...
<button
	type="button"
	class="gap-2-wide hover:bg-hover-solid rounded-button text-tertiary inline-flex items-center px-2 py-1 transition-colors"
	{disabled}
...
>
	...
	{#if count > 0}
		<span class="text-label font-medium">{count}</span>
	{/if}
</button>
