...
describe('workspaces helpers', () => {
	...
	test('createCircleVersionRecord calls recordCreateHistory when circle exists', async () => {
		const recordCreateHistory = vi.spyOn(history, 'recordCreateHistory').mockResolvedValue();
		const ctx = {} as unknown as MutationCtx;

		await createCircleVersionRecord(ctx, { _id: 'c1' } as any);
		expect(recordCreateHistory).toHaveBeenCalledWith(ctx, 'circle', { _id: 'c1' });
	});
	...
	test('createCircleVersionRecord no-ops when circle is null', async () => {
		...
		expect(recordCreateHistory).not.toHaveBeenCalled();
	});
});
