...
// Register custom format for @theme blocks (Tailwind-compatible, no conditional tokens)
StyleDictionary.registerFormat({
	name: 'tailwind/theme-only',
	format: ({ dictionary }) => {
		const transformedTokens = dictionary.allTokens
			.map((token) => transformTailwindTheme(token))
			.filter(Boolean);

		// Filter out conditional tokens - only include regular tokens for @theme block
		const regularTokens = transformedTokens.filter(
			(transformed) =>
				!(transformed && typeof transformed === 'object' && transformed.isConditional)
		);
...
`;
		...
		// Build output: @theme block ONLY (Tailwind v4 compatible)
		let output = header;
...
// Register custom format for conditional tokens (standard CSS, separate file)
StyleDictionary.registerFormat({
	name: 'tailwind/conditional',
	format: ({ dictionary }) => {
		const transformedTokens = dictionary.allTokens
			.map((token) => transformTailwindTheme(token))
			.filter(Boolean);

		// Filter to ONLY conditional tokens
		const conditionalTokens = transformedTokens.filter(
			(transformed) => transformed && typeof transformed === 'object' && transformed.isConditional
		);
...
`;
		...
		// Build output: Standard CSS selectors (not @theme)
		let output = header;
...
// Register legacy format for backward compatibility (used by non-color tokens)
StyleDictionary.registerFormat({
	name: 'tailwind/theme',
	format: ({ dictionary }) => {
		const transformedTokens = dictionary.allTokens
...
export default {
	...
	platforms: {
		css: {
			transformGroup: 'css',
			// Disable reference resolution for conditional tokens (light/dark mode)
			// Style Dictionary will try to resolve {color.syntax.keyword.light} but fails
			// We handle these references in the transform instead
			references: {},
			...
			files: [
				...
				},
				{
					...
					format: 'tailwind/theme-only',
					filter: (token) => token.path[0] === 'color'
				},
				{
					destination: 'colors-conditional.css',
					format: 'tailwind/conditional',
					filter: (token) => token.path[0] === 'color'
				},
				{
