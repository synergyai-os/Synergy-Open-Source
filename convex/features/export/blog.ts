...
export const exportNoteToBlog = action({
	...
	handler: async (ctx, args): Promise<{ filepath: string; content: string; success: boolean }> => {
		...
		// Get the note
		const note = await ctx.runQuery(api.features.notes.index.findNote, {
			sessionId: args.sessionId,
			noteId: args.noteId
		});
		...
		await ctx.runMutation(api.features.notes.index.updateNotePublished, {
			sessionId: args.sessionId,
			noteId: args.noteId,
...
export const listBlogPosts = action({
	...
	handler: async (ctx, args) => {
		...
		// blogExport -> api.features.notes.index -> api (includes blogExport) = circular
		...
		const listNotesQuery = api.features.notes.index.listNotes as FunctionReference<
			'query',
			'public',
