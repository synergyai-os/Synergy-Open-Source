...
- ✅ Domain-specific (Inbox, Meetings, Org Chart, etc.)
- ✅ Business logic included (data fetching, mutations, validation)
- ✅ Built on Design System atoms
- ✅ Module-specific composables for state management
- ✅ Connected to backend (Convex queries/mutations)
...
├── composables/        # State management (.svelte.ts)
├── types.ts            # Module-specific types
└── api.ts              # Module API contract (if exposed)
```
...
<script>
	...
	const data = useTasks({ agendaItemId, sessionId, workspaceId });
</script>
...
- ✅ Consistent styling (design tokens cascade)
- ✅ Accessibility (atoms are WCAG compliant)
- ✅ Type safety (shared types)
- ✅ Easy updates (change atom, updates module)
