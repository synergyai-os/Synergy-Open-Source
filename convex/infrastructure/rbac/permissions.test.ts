...
// From convex/infrastructure/rbac/, ../../../ goes to root, then tests/convex/integration/test.setup
import { modules } from '../../../tests/convex/integration/test.setup';
import { assignRoleToUser } from '../../../tests/convex/integration/setup';
...
describe('RBAC Permission System', () => {
	test("Admin has all permissions with 'all' scope", async () => {
		...
		// Seed RBAC data
		await t.mutation(api.infrastructure.rbac.seedRBAC.seedRBAC, {});
		...
		// SYOS-862: Use assignRoleToUser helper (systemRoles table)
		await assignRoleToUser(t, userId, roleId, { assignedBy: userId });
	...
	test('Circle Lead can only manage own circles', async () => {
		...
		// SYOS-862: Use assignRoleToUser helper (systemRoles table)
		await assignRoleToUser(t, circleLeadId, roleId, { assignedBy: circleLeadId });
		...
		// SYOS-862: Assign as system role - circleId context tested in permission check, not role assignment
		await assignRoleToUser(t, circleLeadId, roleId, {
	...
	test('Multi-role user has permissions from all roles', async () => {
		...
		// SYOS-862: Use assignRoleToUser helper (systemRoles table)
		await assignRoleToUser(t, userId, circleLeadRoleId, { assignedBy: userId });
		await assignRoleToUser(t, userId, billingRoleId, { assignedBy: userId });
