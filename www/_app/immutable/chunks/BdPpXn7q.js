...
import { T as b, U as k, V as l, j as A, a1 as B, a4 as z } from './kOT-I_MI.js';
function r(e) {
	let t = b(k(e));
	return {
		[N]: !0,
...
function q(e = 'ltr', t = 'horizontal') {
	return { horizontal: e === 'rtl' ? g : m, vertical: h }[t];
}
function V(e = 'ltr', t = 'horizontal') {
	return { horizontal: e === 'rtl' ? m : g, vertical: f }[t];
}
function _(e = 'ltr', t = 'horizontal') {
	return (
		['ltr', 'rtl'].includes(e) || (e = 'ltr'),
		['horizontal', 'vertical'].includes(t) || (t = 'horizontal'),
		{ nextKey: q(e, t), prevKey: V(e, t) }
	);
}
...
class nt {
	...
	handleKeydown(t, n, i = !1) {
		const c = this.#t.rootNode.current;
		if (!c || !t) return;
		const s = this.getCandidateNodes();
		if (!s.length) return;
		const u = s.indexOf(t),
			w = M(c),
			{ nextKey: p, prevKey: y } = _(w, this.#t.orientation.current),
			x = this.#t.loop.current,
			d = { [p]: u + 1, [y]: u - 1, [O]: 0, [P]: s.length - 1 };
		if (i) {
			const S = p === h ? m : h,
				T = y === f ? g : f;
			((d[S] = u + 1), (d[T] = u - 1));
		...
		let o = d[n.key];
		if (o === void 0) return;
		(n.preventDefault(), o < 0 && x ? (o = s.length - 1) : o === s.length && x && (o = 0));
		const a = s[o];
		if (a) return (a.focus(), (this.#e.current = a.id), this.#t.onCandidateFocus?.(a), a);
	}
	getTabIndex(t) {
		const n = this.getCandidateNodes(),
			i = this.#e.current !== null;
		return t && !i && n[0] === t
	...
	focusCurrentTabStop() {
		...
		if (!t) return;
		const n = this.#t.rootNode.current?.querySelector(`#${t}`);
		!n || !U(n) || n.focus();
	}
}
class $ {
	...
	run(t) {
		...
		if (n) {
			...
			this.#e = window.requestAnimationFrame(() => {
				...
				Promise.allSettled(i.map((c) => c.finished)).then(() => {
					this.#n(t);
				});
			});
		}
...
class rt {
	...
	constructor(t) {
		((this.#t = t),
			l(this.#n, t.open.current, !0),
			(this.#e = t.enabled ?? !0),
			(this.#r = new $({ ref: this.#t.ref, afterTick: this.#t.open })),
			W(
				() => this.#t.open.current,
				(n) => {
					(n && l(this.#n, !0),
						this.#e &&
							this.#r.run(() => {
								n === this.#t.open.current &&
									(this.#t.open.current || l(this.#n, !1), this.#t.onComplete?.());
							}));
				}
			));
	}
