...
function testRegexPattern() {
	...
	const violationFile = path.join(PROJECT_ROOT, 'src/routes/(authenticated)/org/settings/branding/+page.svelte');
	...
	const pattern = /`:root\.org-\$\{[^}]+\}\s*\{[^`]*--color-(accent-primary|accent-hover)[^`]*\}`/gs;
...
function testSvelteAST() {
	...
	const svelteFile = path.join(PROJECT_ROOT, 'src/routes/(authenticated)/org/settings/branding/+page.svelte');
...
function estimatePerformance() {
		...
		try {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) {
					if (!entry.name.startsWith('.') && entry.name !== 'node_modules' && entry.name !== 'dist' && entry.name !== 'build' && entry.name !== '.storybook') {
	...
	const filteredSvelte = svelteFiles.filter(f => !f.includes('_generated') && !f.includes('node_modules'));
	const filteredTsjs = tsjsFiles.filter(f => !f.includes('_generated') && !f.includes('node_modules') && !f.includes('.storybook'));
...
console.log(`Performance estimate: ${test3.toFixed(1)}s ${test3 < 15 ? '✅' : '⚠️  (exceeds 10s target)'}`);
