...
export async function validateSession(ctx: QueryCtx | MutationCtx, userId: Id<'users'>) {
	...
	if (!session) {
		throw createError(ErrorCodes.SESSION_NOT_FOUND, 'Session not found - user must log in');
	}
	...
	if (session.revokedAt && session.revokedAt <= now) {
		throw createError(
			...
			'Session has been revoked - user must log in again'
		);
	}
...
export async function validateSessionAndGetUserId(
	...
	if (!session) {
		throw createError(
			...
			'Session not found or expired - user must log in'
		);
	}
