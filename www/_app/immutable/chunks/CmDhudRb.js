...
const Ne = [];
function ce(s, e = !1, t = !1) {
	return J(s, new Map(), '', Ne, null, t);
}
function J(s, e, t, n, r = null, i = !1) {
	if (typeof s == 'object' && s !== null) {
		...
		if (Oe(s)) {
			...
			(e.set(s, a), r !== null && e.set(r, a));
			for (var u = 0; u < s.length; u += 1) {
				var l = s[u];
				u in s && (a[u] = J(l, e, t, n, null, i));
			}
			return a;
		}
		if (xe(s) === Le) {
			((a = {}), e.set(s, a), r !== null && e.set(r, a));
			for (var p in s) a[p] = J(s[p], e, t, n, null, i);
			return a;
		}
		if (s instanceof Date) return structuredClone(s);
		if (typeof s.toJSON == 'function' && !i) return J(s.toJSON(), e, t, n, s);
	}
...
for (var M = 0, De = Z.length; M < De; ++M) ((k[M] = Z[M]), (S[Z.charCodeAt(M)] = M));
S[45] = 62;
S[95] = 63;
function Ue(s) {
	var e = s.length;
...
function N(s) {
	...
	for (u = 0; u < a; u += 4)
		((e =
			(S[s.charCodeAt(u)] << 18) |
			(S[s.charCodeAt(u + 1)] << 12) |
			(S[s.charCodeAt(u + 2)] << 6) |
			S[s.charCodeAt(u + 3)]),
			(i[o++] = (e >> 16) & 255),
			(i[o++] = (e >> 8) & 255),
			(i[o++] = e & 255));
	return (
		r === 2 &&
			((e = (S[s.charCodeAt(u)] << 2) | (S[s.charCodeAt(u + 1)] >> 4)), (i[o++] = e & 255)),
		r === 1 &&
			((e =
				(S[s.charCodeAt(u)] << 10) | (S[s.charCodeAt(u + 1)] << 4) | (S[s.charCodeAt(u + 2)] >> 2)),
			(i[o++] = (e >> 8) & 255),
			(i[o++] = e & 255)),
		i
	);
...
function je(s) {
	return k[(s >> 18) & 63] + k[(s >> 12) & 63] + k[(s >> 6) & 63] + k[s & 63];
}
function Je(s, e, t) {
	for (var n, r = [], i = e; i < t; i += 3)
		((n = ((s[i] << 16) & 16711680) + ((s[i + 1] << 8) & 65280) + (s[i + 2] & 255)), r.push(je(n)));
	return r.join('');
}
function B(s) {
	for (var e, t = s.length, n = t % 3, r = [], i = 16383, o = 0, a = t - n; o < a; o += i)
		r.push(Je(s, o, o + i > a ? a : o + i));
	return (
		n === 1
			? ((e = s[t - 1]), r.push(k[e >> 2] + k[(e << 4) & 63] + '=='))
			: n === 2 &&
				((e = (s[t - 2] << 8) + s[t - 1]),
				r.push(k[e >> 10] + k[(e >> 4) & 63] + k[(e << 2) & 63] + '=')),
		r.join('')
	);
}
function T(s) {
	if (s === void 0) return {};
	if (!Ce(s))
		throw new Error(`The arguments to a Convex function must be an object. Received: ${s}`);
	return s;
}
function ke(s) {
	if (typeof s > 'u')
	...
	if (typeof s != 'string') throw new Error(`Invalid deployment address: found ${s}".`);
...
function Ce(s) {
	const e = typeof s == 'object',
		t = Object.getPrototypeOf(s),
		n = t === null || t === Object.prototype || t?.constructor?.name === 'Object';
	return e && n;
}
const Re = !0,
	E = BigInt('-9223372036854775808'),
	he = BigInt('9223372036854775807'),
	ie = BigInt('0'),
	He = BigInt('8'),
	ze = BigInt('256');
function Te(s) {
...
function Xe(s) {
	...
	let n = 0;
	for (const r of e.match(/.{2}/g).reverse()) (t.set([parseInt(r, 16)], n++), (s >>= He));
	return B(t);
}
function Ge(s) {
	const e = N(s);
	...
	let t = ie,
		n = ie;
	for (const r of e) ((t += BigInt(r) * ze ** n), n++);
	return (t > he && (t += E + E), t);
}
function Ye(s) {
	if (s < E || he < s) throw new Error(`BigInt ${s} does not fit into a 64-bit signed integer.`);
	const e = new ArrayBuffer(8);
	return (new DataView(e).setBigInt64(0, s, !0), B(new Uint8Array(e)));
}
function Ke(s) {
	const e = N(s);
	if (e.byteLength !== 8)
...
const Ze = DataView.prototype.setBigInt64 ? Ye : Xe,
	et = DataView.prototype.getBigInt64 ? Ke : Ge,
	de = 1024;
function Ae(s) {
	if (s.length > de) throw new Error(`Field name ${s} exceeds maximum field name length ${de}.`);
	if (s.startsWith('$')) throw new Error(`Field name ${s} starts with a '$', which is reserved.`);
	for (let e = 0; e < s.length; e += 1) {
		const t = s.charCodeAt(e);
		if (t < 32 || t >= 127)
			throw new Error(
				`Field name ${s} has invalid character '${s[e]}': Field names can only contain non-control ASCII characters`
			);
	}
}
function O(s) {
	if (s === null || typeof s == 'boolean' || typeof s == 'number' || typeof s == 'string') return s;
	if (Array.isArray(s)) return s.map((n) => O(n));
	if (typeof s != 'object') throw new Error(`Unexpected type of ${s}`);
	const e = Object.entries(s);
	if (e.length === 1) {
		const n = e[0][0];
		if (n === '$bytes') {
			if (typeof s.$bytes != 'string') throw new Error(`Malformed $bytes field on ${s}`);
			return N(s.$bytes).buffer;
		}
		if (n === '$integer') {
			if (typeof s.$integer != 'string') throw new Error(`Malformed $integer field on ${s}`);
			return et(s.$integer);
		}
		if (n === '$float') {
			if (typeof s.$float != 'string') throw new Error(`Malformed $float field on ${s}`);
			const r = N(s.$float);
			if (r.byteLength !== 8)
				throw new Error(`Received ${r.byteLength} bytes, expected 8 for $float`);
			const o = new DataView(r.buffer).getFloat64(0, Re);
			if (!Te(o)) throw new Error(`Float ${o} should be encoded as a number`);
			return o;
		}
		if (n === '$set')
	...
	const t = {};
	for (const [n, r] of Object.entries(s)) (Ae(n), (t[n] = O(r)));
	return t;
}
function F(s) {
...
function oe(s, e, t, n) {
	if (s === void 0) {
		const o = t && ` (present at path ${t} in original object ${F(e)})`;
		throw new Error(
	...
	if (typeof s == 'number')
		if (Te(s)) {
			...
			return (new DataView(o).setFloat64(0, s, Re), { $float: B(new Uint8Array(o)) });
		} else return s;
	if (typeof s == 'boolean' || typeof s == 'string') return s;
	if (s instanceof ArrayBuffer) return { $bytes: B(new Uint8Array(s)) };
	if (Array.isArray(s)) return s.map((o, a) => oe(o, e, t + `[${a}]`));
	if (s instanceof Set) throw new Error(ee(t, 'Set', [...s], e));
	if (s instanceof Map) throw new Error(ee(t, 'Map', [...s], e));
	if (!Ce(s)) {
		const o = s?.constructor?.name,
			a = o ? `${o} ` : '';
		throw new Error(ee(t, a, s, e));
	}
	const r = {},
		i = Object.entries(s);
	i.sort(([o, a], [u, l]) => (o === u ? 0 : o < u ? -1 : 1));
	for (const [o, a] of i) a !== void 0 && (Ae(o), (r[o] = oe(a, e, t + `.${o}`)));
	return r;
}
function ee(s, e, t, n) {
	return s
		? `${e}${F(t)} is not a supported Convex type (present at path ${s} in original object ${F(n)}). To learn about Convex's supported types, see https://docs.convex.dev/using/types.`
		: `${e}${F(t)} is not a supported Convex type.`;
}
function R(s) {
	return oe(s, s, '');
}
var tt = Object.defineProperty,
	st = (s, e, t) =>
		e in s ? tt(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	te = (s, e, t) => st(s, typeof e != 'symbol' ? e + '' : e, t),
	fe,
	ge;
const nt = Symbol.for('ConvexError');
class ae extends ((ge = Error), (fe = nt), ge) {
	constructor(e) {
		(super(typeof e == 'string' ? e : F(e)),
			te(this, 'name', 'ConvexError'),
			te(this, 'data'),
			te(this, fe, !0),
			(this.data = e));
	}
...
var rt = Object.defineProperty,
	it = (s, e, t) =>
		e in s ? rt(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	ye = (s, e, t) => it(s, typeof e != 'symbol' ? e + '' : e, t);
const ot = 'color:rgb(0, 145, 255)';
function qe(s) {
	switch (s) {
		case 'query':
			return 'Q';
...
class _e {
	...
	addLogLineListener(e) {
		...
		for (let n = 0; n < 10 && this._onLogLineFuncs[t] !== void 0; n++)
...
function X(s, e, t, n, r) {
	const i = qe(t);
	if (
		(typeof r == 'object' && (r = `ConvexError ${JSON.stringify(r.errorData, null, 2)}`),
		e === 'info')
	) {
		const o = r.match(/^\[.*?\] /);
		if (o === null) {
			s.error(`[CONVEX ${i}(${n})] Could not parse console.log`);
			return;
		}
		const a = r.slice(1, o[0].length - 2),
			u = r.slice(o[0].length);
		s.log(`%c[CONVEX ${i}(${n})] [${a}]`, ot, u);
	} else s.error(`[CONVEX ${i}(${n})] ${r}`);
}
function ct(s, e) {
	const t = `[CONVEX FATAL ERROR] ${e}`;
	return (s.error(t), new Error(t));
}
function $(s, e, t) {
	return `[CONVEX ${qe(s)}(${e})] ${t.errorMessage}
  Called by client`;
}
function ue(s, e) {
	return ((e.data = s.errorData), e);
}
function G(s) {
	const e = s.split(':');
	let t, n;
	return (
		e.length === 1
			? ((t = e[0]), (n = 'default'))
			: ((t = e.slice(0, e.length - 1).join(':')), (n = e[e.length - 1])),
		t.endsWith('.js') && (t = t.slice(0, -3)),
		`${t}:${n}`
	);
}
function I(s, e) {
	return JSON.stringify({ udfPath: G(s), args: R(e) });
}
var ht = Object.defineProperty,
	lt = (s, e, t) =>
		e in s ? ht(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	v = (s, e, t) => lt(s, typeof e != 'symbol' ? e + '' : e, t);
class dt {
	constructor() {
		(v(this, 'nextQueryId'),
			v(this, 'querySetVersion'),
			v(this, 'querySet'),
			v(this, 'queryIdToToken'),
			v(this, 'identityVersion'),
			v(this, 'auth'),
			v(this, 'outstandingQueriesOlderThanRestart'),
			v(this, 'outstandingAuthOlderThanRestart'),
			v(this, 'paused'),
			v(this, 'pendingQuerySetModifications'),
			(this.nextQueryId = 0),
	...
	subscribe(e, t, n, r) {
			...
			);
		{
			...
			const p = this.querySetVersion,
				w = this.querySetVersion + 1,
				C = { type: 'Add', queryId: u, udfPath: i, args: [R(t)], journal: n, componentPath: r };
			return (
				this.paused ? this.pendingQuerySetModifications.set(u, C) : (this.querySetVersion = w),
				{
					queryToken: o,
					modification: {
						type: 'ModifyQuerySet',
		...
		for (const t of e.modifications)
			switch (t.type) {
				...
				case 'QueryFailed': {
					...
					const n = t.journal;
					if (n !== void 0) {
						const r = this.queryIdToToken.get(t.queryId);
						r !== void 0 && (this.querySet.get(r).journal = n);
					}
					break;
				}
				case 'QueryRemoved': {
	...
	setAdminAuth(e, t) {
		const n = { tokenType: 'Admin', value: e, impersonating: t };
		this.auth = n;
	...
	restart(e) {
		...
		for (const i of this.querySet.values()) {
			...
			(t.push(o), e.has(i.id) || this.outstandingQueriesOlderThanRestart.add(i.id));
		}
		this.querySetVersion = 1;
		const n = { type: 'ModifyQuerySet', baseVersion: 0, newVersion: 1, modifications: t };
	...
	removeSubscriber(e) {
		...
		if (t.numSubscribers > 1) return ((t.numSubscribers -= 1), null);
		{
			...
			return (
				...
				{ type: 'ModifyQuerySet', baseVersion: n, newVersion: r, modifications: [i] }
			);
		}
	}
}
var ft = Object.defineProperty,
	gt = (s, e, t) =>
		e in s ? ft(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	W = (s, e, t) => gt(s, typeof e != 'symbol' ? e + '' : e, t);
class yt {
	constructor(e, t) {
		((this.logger = e),
			(this.markConnectionStateDirty = t),
			W(this, 'inflightRequests'),
			W(this, 'requestsOlderThanRestart'),
			W(this, 'inflightMutationsCount', 0),
			W(this, 'inflightActionsCount', 0),
			(this.inflightRequests = new Map()),
			(this.requestsOlderThanRestart = new Set()));
	}
	request(e, t) {
		const n = new Promise((r) => {
			const i = t ? 'Requested' : 'NotSent';
			(this.inflightRequests.set(e.requestId, {
				message: e,
				status: { status: i, requestedAt: new Date(), onResult: r }
			}),
				e.type === 'Mutation'
	...
	onResponse(e) {
		...
		const n = t.message.type === 'Mutation' ? 'mutation' : 'action',
			r = t.message.udfPath;
		for (const u of e.logLines) X(this.logger, 'info', n, r, u);
		const i = t.status;
		let o, a;
		if (e.success)
			((o = { success: !0, logLines: e.logLines, value: O(e.result) }), (a = () => i.onResult(o)));
		else {
			const u = e.result,
				{ errorData: l } = e;
			(X(this.logger, 'error', n, r, u),
				(o = {
					success: !1,
					errorMessage: u,
					errorData: l !== void 0 ? O(l) : void 0,
					logLines: e.logLines
				}),
				(a = () => i.onResult(o)));
		}
	...
	removeCompleted(e) {
		const t = new Map();
		for (const [n, r] of this.inflightRequests.entries()) {
			const i = r.status;
			i.status === 'Completed' &&
	...
	restart() {
		...
		const e = [];
		for (const [t, n] of this.inflightRequests) {
...
const D = Symbol.for('functionName'),
	pt = Symbol.for('toReferencePath');
function mt(s) {
	return s[pt] ?? null;
}
function wt(s) {
...
function bt(s) {
	let e;
	if (typeof s == 'string') wt(s) ? (e = { functionHandle: s }) : (e = { name: s });
	else if (s[D]) e = { name: s[D] };
	else {
		const t = mt(s);
		if (!t) throw new Error(`${s} is not a functionReference`);
		e = { reference: t };
	}
	return e;
}
...
var St = Object.defineProperty,
	...
	Y = (s, e, t) => vt(s, typeof e != 'symbol' ? e + '' : e, t);
class U {
	constructor(e) {
		(Y(this, 'queryResults'),
			Y(this, 'modifiedQueries'),
			(this.queryResults = e),
	...
	getQuery(e, ...t) {
		const n = T(t[0]),
			r = A(e),
			i = this.queryResults.get(I(r, n));
		if (i !== void 0) return U.queryValue(i.result);
	}
	getAllQueries(e) {
		const t = [],
			n = A(e);
		for (const r of this.queryResults.values())
			r.udfPath === G(n) && t.push({ args: r.args, value: U.queryValue(r.result) });
		return t;
	}
	setQuery(e, t, n) {
		const r = T(t),
			...
			o = I(i, r);
		let a;
		n === void 0 ? (a = void 0) : (a = { success: !0, value: n, logLines: [] });
		const u = { udfPath: i, args: r, result: a };
	...
	queryResult(e) {
		...
		if (n !== void 0) {
			...
			throw n.errorData !== void 0
				? ue(n, new ae($('query', t.udfPath, n)))
				: new Error($('query', t.udfPath, n));
		}
	}
	hasQueryResult(e) {
...
var Ct = Object.defineProperty,
	Rt = (s, e, t) =>
		e in s ? Ct(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	se = (s, e, t) => Rt(s, typeof e != 'symbol' ? e + '' : e, t);
class m {
	constructor(e, t) {
		(se(this, 'low'),
			se(this, 'high'),
			se(this, '__isUnsignedLong__'),
			(this.low = e | 0),
			(this.high = t | 0),
	...
	static fromNumber(e) {
		return isNaN(e) || e < 0 ? pe : e >= Tt ? At : new m(e % Q | 0, (e / Q) | 0);
	}
	toString() {
		return (BigInt(this.high) * BigInt(Q) + BigInt(this.low)).toString();
	}
	equals(e) {
		return (
			m.isLong(e) || (e = m.fromValue(e)),
	...
	static fromValue(e) {
		return typeof e == 'number' ? m.fromNumber(e) : new m(e.low, e.high);
	}
}
const pe = new m(0, 0),
	me = 65536,
...
var qt = Object.defineProperty,
	_t = (s, e, t) =>
		e in s ? qt(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	j = (s, e, t) => _t(s, typeof e != 'symbol' ? e + '' : e, t);
class we {
	constructor(e, t) {
		(j(this, 'version'),
			...
			j(this, 'queryPath'),
			j(this, 'logger'),
			(this.version = { querySet: 0, ts: m.fromNumber(0), identity: 0 }),
	...
	transition(e) {
		...
		for (const n of e.modifications)
			switch (n.type) {
				case 'QueryUpdated': {
					const r = this.queryPath(n.queryId);
					if (r) for (const o of n.logLines) X(this.logger, 'info', 'query', r, o);
					const i = O(n.value ?? null);
					this.remoteQuerySet.set(n.queryId, { success: !0, value: i, logLines: n.logLines });
					break;
				}
				case 'QueryFailed': {
					const r = this.queryPath(n.queryId);
					if (r) for (const o of n.logLines) X(this.logger, 'info', 'query', r, o);
					const { errorData: i } = n;
					this.remoteQuerySet.set(n.queryId, {
						success: !1,
						errorMessage: n.errorMessage,
						errorData: i !== void 0 ? O(i) : void 0,
						logLines: n.logLines
					});
					break;
				}
				case 'QueryRemoved': {
					this.remoteQuerySet.delete(n.queryId);
					break;
				}
				default:
...
function ne(s) {
	const e = N(s);
	return m.fromBytesLE(Array.from(e));
}
function It(s) {
	const e = new Uint8Array(s.toBytesLE());
	return B(e);
}
function be(s) {
	switch (s.type) {
		...
		case 'MutationResponse':
			return s.success ? { ...s, ts: ne(s.ts) } : { ...s };
		case 'Transition':
			return {
				...s,
				startVersion: { ...s.startVersion, ts: ne(s.startVersion.ts) },
				endVersion: { ...s.endVersion, ts: ne(s.endVersion.ts) }
			};
	}
}
function Mt(s) {
	switch (s.type) {
		...
		case 'Connect':
			return s.maxObservedTimestamp !== void 0
				? { ...s, maxObservedTimestamp: It(s.maxObservedTimestamp) }
...
var $t = Object.defineProperty,
	...
	g = (s, e, t) => Et(s, typeof e != 'symbol' ? e + '' : e, t);
const Ot = 1e3,
	xt = 1001,
	Lt = 1005,
	Vt = 4040;
...
function z() {
	return (
		H === void 0 && (H = Date.now()),
		typeof performance > 'u' || !performance.now ? Date.now() : Math.round(H + performance.now())
	);
}
function Se() {
	return `t=${Math.round((z() - H) / 100) / 10}s`;
}
const Me = {
...
function Pt(s) {
	...
	for (const e of Object.keys(Me)) if (s.startsWith(e)) return e;
	return 'Unknown';
}
class Ft {
	constructor(e, t, n, r, i, o) {
		((this.markConnectionStateDirty = i),
			(this.debug = o),
			g(this, 'socket'),
			g(this, 'connectionCount'),
			g(this, '_hasEverConnected', !1),
			g(this, 'lastCloseReason'),
			g(this, 'transitionChunkBuffer', null),
			g(this, 'defaultInitialBackoff'),
			g(this, 'maxBackoff'),
			g(this, 'retries'),
			g(this, 'serverInactivityThreshold'),
			g(this, 'reconnectDueToServerInactivityTimeout'),
			g(this, 'uri'),
			g(this, 'onOpen'),
			g(this, 'onResume'),
			g(this, 'onMessage'),
			g(this, 'webSocketConstructor'),
			g(this, 'logger'),
			g(this, 'onServerDisconnectError'),
	...
	assembleTransition(e) {
		...
		if (
			...
			this.transitionChunkBuffer = null;
			const n = be(JSON.parse(t));
			if (n.type !== 'Transition')
				throw new Error(`Expected Transition, got ${n.type} after assembling chunks`);
			return n;
		}
		return null;
	}
	connect() {
		...
		(this._logVerbose('constructed WebSocket'),
				...
				(this.setSocketState({
					...
					this.lastCloseReason !== 'InitialConnect' &&
						(this.lastCloseReason
							...
							: this.logger.log('WebSocket reconnected at', Se())),
			...
			(e.onerror = (t) => {
				this.transitionChunkBuffer = null;
				const n = t.message;
				n && this.logger.log(`WebSocket error message: ${n}`);
			}),
			(e.onmessage = (t) => {
				this.resetServerInactivityTimeout();
				const n = t.data.length;
				let r = be(JSON.parse(t.data));
				if ((this._logVerbose(`received ws message with type ${r.type}`), r.type === 'Ping'))
					return;
				if (r.type === 'TransitionChunk') {
					const o = this.assembleTransition(r);
					if (!o) return;
					((r = o), this._logVerbose(`assembled full ws message of type ${r.type}`));
				}
				(this.transitionChunkBuffer !== null &&
					((this.transitionChunkBuffer = null),
					this.logger.log(`Received unexpected ${r.type} while buffering TransitionChunks`)),
					r.type === 'Transition' &&
						this.reportLargeTransition({ messageLength: n, transition: r }),
			...
			(e.onclose = (t) => {
				if (
					...
					t.code !== Ot && t.code !== xt && t.code !== Lt && t.code !== Vt)
				) {
					let r = `WebSocket closed with code ${t.code}`;
					(t.reason && (r += `: ${t.reason}`),
						this.logger.log(r),
						this.onServerDisconnectError && t.reason && this.onServerDisconnectError(r));
				}
				const n = Pt(t.reason);
	...
	nextBackoff(e) {
		const n =
			(e === 'client' ? 100 : e === 'Unknown' ? this.defaultInitialBackoff : Me[e].timeout) *
	...
	reportLargeTransition({ transition: e, messageLength: t }) {
		...
		const n = z() - e.clientClockSkew - e.serverTs / 1e6,
			r = `${Math.round(n)}ms`,
			i = `${Math.round(t / 1e4) / 100}MB`,
			o = t / (n / 1e3),
			a = `${Math.round(o / 1e4) / 100}MB per second`;
		(this._logVerbose(`received ${i} transition in ${r} at ${a}`),
			t > 2e7
				? this.logger.log(
...
function $e(s, e) {
	if (typeof s != 'string') throw new P('Invalid token specified: must be a string');
	e || (e = {});
	const t = e.header === !0 ? 0 : 1,
		n = s.split('.')[t];
	if (typeof n != 'string') throw new P(`Invalid token specified: missing part #${t + 1}`);
	let r;
	try {
		r = Dt(n);
	} catch (i) {
		throw new P(`Invalid token specified: invalid base64 for part #${t + 1} (${i.message})`);
	}
	try {
		return JSON.parse(r);
	} catch (i) {
		throw new P(`Invalid token specified: invalid json for part #${t + 1} (${i.message})`);
	}
}
var Ut = Object.defineProperty,
	Wt = (s, e, t) =>
		e in s ? Ut(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	b = (s, e, t) => Wt(s, typeof e != 'symbol' ? e + '' : e, t);
const jt = 480 * 60 * 60 * 1e3,
	ve = 2;
class Jt {
	constructor(e, t, n) {
		(b(this, 'authState', { state: 'noAuth' }),
			b(this, 'configVersion', 0),
			b(this, 'syncState'),
			b(this, 'authenticate'),
			b(this, 'stopSocket'),
			b(this, 'tryRestartSocket'),
			b(this, 'pauseSocket'),
			b(this, 'resumeSocket'),
			b(this, 'clearAuth'),
			b(this, 'logger'),
			b(this, 'refreshTokenLeewaySeconds'),
			b(this, 'tokenConfirmationAttempts', 0),
			(this.syncState = e),
	...
	async setConfig(e, t) {
		(this.resetAuthState(),
			...
			this.pauseSocket());
		const n = await this.fetchTokenAndGuardAgainstRace(e, { forceRefreshToken: !1 });
	...
	async tryToReauthenticate(e) {
		if (
			...
			this.authState.state === 'noAuth' ||
				(this.authState.state === 'waitingForServerConfirmationOfFreshToken' &&
					this.tokenConfirmationAttempts >= ve))
		) {
		...
		(this.authState.state === 'waitingForServerConfirmationOfFreshToken' &&
			...
			this._logVerbose(
				`retrying reauthentication, ${ve - this.tokenConfirmationAttempts} attempts remaining`
			)),
	...
	scheduleTokenRefetch(e) {
		...
		let o = Math.min(jt, (i - this.refreshTokenLeewaySeconds) * 1e3);
		o <= 0 &&
			(this.logger.warn(
	...
	async fetchTokenAndGuardAgainstRace(e, t) {
		const n = ++this.configVersion;
		this._logVerbose(`fetching token with config version ${n}`);
		const r = await e(t);
		return this.configVersion !== n
			? (this._logVerbose(`stale config version, expected ${n}, got ${this.configVersion}`),
...
const Ht = ['convexClientConstructed', 'convexWebSocketOpen', 'convexFirstMessageReceived'];
function zt(s, e) {
	const t = { sessionId: e };
	typeof performance > 'u' || !performance.mark || performance.mark(s, { detail: t });
}
function Xt(s) {
	let e = s.name.slice(6);
	return ((e = e.charAt(0).toLowerCase() + e.slice(1)), { name: e, startTime: s.startTime });
}
function Gt(s) {
	...
	for (const t of Ht) {
		const n = performance
			.getEntriesByName(t)
			.filter((r) => r.entryType === 'mark')
			.filter((r) => r.detail.sessionId === s);
		e.push(...n);
	}
	return e.map(Xt);
}
var Yt = Object.defineProperty,
	Kt = (s, e, t) =>
		e in s ? Yt(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	d = (s, e, t) => Kt(s, typeof e != 'symbol' ? e + '' : e, t);
class Zt {
	constructor(e, t, n) {
		if (
			(d(this, 'address'),
			...
			d(this, 'webSocketManager'),
			d(this, 'authenticationManager'),
			d(this, 'remoteQuerySet'),
			d(this, 'optimisticQueryResults'),
			d(this, '_transitionHandlerCounter', 0),
			d(this, '_nextRequestId'),
			d(this, '_onTransitionFns', new Map()),
			d(this, '_sessionId'),
			d(this, 'firstMessageReceived', !1),
			d(this, 'debug'),
			d(this, 'logger'),
			d(this, 'maxObservedTimestamp'),
			d(this, 'connectionStateSubscribers', new Map()),
			d(this, 'nextConnectionStateSubscriberId', 0),
			d(this, '_lastPublishedConnectionState'),
			d(this, 'markConnectionStateDirty', () => {
			...
			d(this, 'mark', (c) => {
				this.debug && zt(c, this.sessionId);
			}),
			typeof e == 'object')
		)
			throw new Error(
		...
		(n?.skipConvexDeploymentUrlCheck !== !0 && ke(e), (n = { ...n }));
		const r = n.authRefreshTokenLeewaySeconds ?? 2;
		let i = n.webSocketConstructor;
		...
		((i = i || WebSocket),
			...
			(this.logger =
				n.logger === !1
					? ut({ verbose: n.verbose ?? !1 })
					: n.logger !== !0 && n.logger
						? n.logger
						: at({ verbose: n.verbose ?? !1 })));
		const o = e.search('://');
		...
		const p = `${l}://${a}/api/${le}/sync`;
		((this.state = new dt()),
			(this.remoteQuerySet = new we((c) => this.state.queryPath(c), this.logger)),
			(this.requestManager = new yt(this.logger, this.markConnectionStateDirty)));
		const w = () => {
			(this.webSocketManager.pause(), this.state.pause());
		};
		((this.authenticationManager = new Jt(
			this.state,
			{
				authenticate: (c) => {
			...
			(this.optimisticQueryResults = new kt()),
			this.addOnTransitionHandler((c) => {
				t(c.queries.map((h) => h.token));
			}),
			(this._nextRequestId = 0),
			(this._sessionId = Qt()));
		const { unsavedChangesWarning: C } = n;
		...
		((this.webSocketManager = new Ft(
			p,
			{
				onOpen: (c) => {
					...
					this.remoteQuerySet = new we((K) => this.state.queryPath(K), this.logger);
					const [f, q] = this.state.restart(h);
					(q && this.webSocketManager.sendMessage(q), this.webSocketManager.sendMessage(f));
					for (const K of this.requestManager.restart()) this.webSocketManager.sendMessage(K);
				},
				onResume: () => {
					const [c, h] = this.state.resume();
					(h && this.webSocketManager.sendMessage(h), c && this.webSocketManager.sendMessage(c));
					for (const f of this.requestManager.resume()) this.webSocketManager.sendMessage(f);
				},
				onMessage: (c) => {
					switch (
						...
						case 'FatalError': {
							const h = ct(this.logger, c.error);
	...
	notifyOnQueryResultChanges(e) {
		...
		for (const [i, o] of t) {
			...
			if (a !== null) {
				const u = { result: o, udfPath: this.state.queryPath(i), args: this.state.queryArgs(i) };
				n.set(a, u);
			}
		}
		const r = this.optimisticQueryResults.ingestQueryResultsFromServer(n, new Set(e.keys()));
		this.handleTransition({
			queries: r.map((i) => {
				const o = this.optimisticQueryResults.rawQueryResult(i);
				return { token: i, modification: { kind: 'Updated', result: o.result } };
			}),
			reflectedMutations: Array.from(e).map(([i, o]) => ({ requestId: i, result: o })),
	...
	setAdminAuth(e, t) {
		const n = this.state.setAdminAuth(e, t);
	...
	async mutation(e, t, n) {
		const r = await this.mutationInternal(e, t, n);
		if (!r.success)
			throw r.errorData !== void 0
				? ue(r, new ae($('mutation', e, r)))
				: new Error($('mutation', e, r));
		return r.value;
	}
	async mutationInternal(e, t, n, r) {
		const { mutationPromise: i } = this.enqueueMutation(e, t, n, r);
		return i;
	}
	enqueueMutation(e, t, n, r) {
		...
		if ((this._nextRequestId++, n !== void 0)) {
			const p = n.optimisticUpdate;
			if (p !== void 0) {
				const w = (h) => {
						p(h, i) instanceof Promise &&
							this.logger.warn(
								'Optimistic update handler returned a Promise. Optimistic updates should be synchronous.'
							);
					},
					c = this.optimisticQueryResults.applyOptimisticUpdate(w, o).map((h) => {
						const f = this.localQueryResultByToken(h);
						return {
							token: h,
							modification: {
								kind: 'Updated',
								result: f === void 0 ? void 0 : { success: !0, value: f, logLines: [] }
							}
						};
					});
				this.handleTransition({
		...
		const a = { type: 'Mutation', requestId: o, udfPath: e, componentPath: r, args: [R(i)] },
			u = this.webSocketManager.sendMessage(a),
			l = this.requestManager.request(a, u);
		return { requestId: o, mutationPromise: l };
	}
	async action(e, t) {
		const n = await this.actionInternal(e, t);
		if (!n.success)
			throw n.errorData !== void 0
				? ue(n, new ae($('action', e, n)))
				: new Error($('action', e, n));
		return n.value;
	}
	async actionInternal(e, t, n) {
		...
		const o = { type: 'Action', requestId: i, udfPath: e, componentPath: n, args: [R(r)] },
	...
	tryReportLongDisconnect() {
		...
		fetch(t, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', 'Convex-Client': `npm-${le}` },
			body: JSON.stringify({ event: 'LongWebsocketDisconnect' })
		})
			.then((n) => {
				n.ok || this.logger.warn('Analytics request failed with response:', n.body);
			})
			.catch((n) => {
				this.logger.warn('Analytics response failed with error:', n);
			});
	}
}
var es = Object.defineProperty,
	ts = (s, e, t) =>
		e in s ? es(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t),
	x = (s, e, t) => ts(s, typeof e != 'symbol' ? e + '' : e, t);
class ss {
	constructor(e, t = {}) {
		(x(this, 'listeners'),
			x(this, '_client'),
			x(this, 'callNewListenersWithCurrentValuesTimer'),
			x(this, '_closed'),
			x(this, '_disabled'),
			t.skipConvexDeploymentUrlCheck !== !0 && ke(e));
		const { disabled: n, ...r } = t;
		((this._closed = !1),
			(this._disabled = !!n),
			typeof window > 'u' && !('unsavedChangesWarning' in r) && (r.unsavedChangesWarning = !1),
			this.disabled || (this._client = new Zt(e, (i) => this._transition(i), r)),
	...
	onUpdate(e, t, n, r) {
		if (this.disabled) {
			const p = () => {};
			return (
				Object.assign(p, { unsubscribe: p, getCurrentValue: () => {}, getQueryLogs: () => {} }),
				p
			);
		}
		const { queryToken: i, unsubscribe: o } = this.client.subscribe(A(e), t),
			a = {
				queryToken: i,
				callback: n,
				onError: r,
		...
		const u = {
				unsubscribe: () => {
					this.closed || (this.listeners.delete(a), o());
				},
	...
	callNewListenersWithCurrentValues() {
		((this.callNewListenersWithCurrentValuesTimer = void 0), this._transition([], !0));
	}
	queryResultReady(e) {
		return this.client.hasLocalQueryResultByToken(e);
	}
	async close() {
		if (!this.disabled) return (this.listeners.clear(), (this._closed = !0), this.client.close());
	}
	getAuth() {
	...
	_transition(e, t = !1) {
		for (const n of this.listeners) {
			const { callback: r, queryToken: i, onError: o, hasEverRun: a } = n;
			if (e.includes(i) || (t && !a && this.client.hasLocalQueryResultByToken(i))) {
				n.hasEverRun = !0;
				let u;
				try {
					...
					o
						? o(l, 'Second argument to onUpdate onError is reserved for later use')
						: Promise.reject(l);
					continue;
				}
				r(u, 'Second argument to onUpdate callback is reserved for later use');
			}
		}
	}
	async mutation(e, t, n) {
		if (this.disabled) throw new Error('ConvexClient is disabled');
		return await this.client.mutation(A(e), t, n);
	}
	async action(e, t) {
		if (this.disabled) throw new Error('ConvexClient is disabled');
		return await this.client.action(A(e), t);
	}
	async query(e, t) {
		if (this.disabled) throw new Error('ConvexClient is disabled');
		const n = this.client.localQueryResult(A(e), t);
		return n !== void 0
			? Promise.resolve(n)
			: new Promise((r, i) => {
...
function cs(s, e, t = {}) {
	...
	const r = Pe({
		result: V(t).initialData,
		argsForLastResult: void 0,
		lastResult: void 0,
		haveArgsEverChanged: !1
	});
	re(() => {
		const c = L(e);
		return n.onUpdate(
			...
			(f) => {
				...
				((r.result = q), (r.argsForLastResult = c), (r.lastResult = q));
			},
			(f) => {
				((r.result = f), (r.argsForLastResult = c));
				const q = structuredClone(f);
				r.lastResult = q;
			}
		);
	...
	const i = _(
			() =>
				!!r.argsForLastResult && JSON.stringify(R(r.argsForLastResult)) === JSON.stringify(R(L(e)))
		),
		o = _(() => !!(V(t).keepPreviousData && r.lastResult)),
		a = L(e);
	re(() => {
		Fe(() => r.haveArgsEverChanged) ||
			(JSON.stringify(R(L(e))) !== JSON.stringify(R(a)) &&
				((r.haveArgsEverChanged = !0),
				V(t).initialData !== void 0 &&
					((r.argsForLastResult = ce(a)), (r.lastResult = V(t).initialData))));
	});
	const u = _(() => {
			if (V(t).initialData && !r.haveArgsEverChanged) return r.result;
			let h;
			try {
				h = n.disabled ? void 0 : n.client.localQueryResult(A(s), L(e));
			} catch (f) {
				if (!(f instanceof Error)) throw (console.error('threw non-Error instance', f), f);
				h = f;
			}
			return (r.result, h);
		}),
		l = _(() => (y(u) !== void 0 ? y(u) : y(o) ? r.lastResult : void 0)),
		p = _(() => y(u) === void 0 && y(o) && !y(i) && y(l) !== void 0),
		w = _(() => {
			if (!(y(l) instanceof Error)) return y(l);
		}),
		C = _(() => {
			if (y(l) instanceof Error) return y(l);
		});
...
function L(s) {
	return (typeof s == 'function' && (s = s()), ce(s));
}
function V(s) {
	return (typeof s == 'function' && (s = s()), ce(s));
}
export { cs as a, as as b, os as m, us as s, pt as t, ns as u };
