...
test.describe('Inbox Sync Workflow', () => {
	...
	test('user can open sync configuration', async ({ page }) => {
		...
		const menuButton = page.locator('button').filter({ has: page.locator('svg path[d*="M12 5v.01M12 12v.01M12 19v.01"]') }).first();
		...
		await expect(
			page.locator('text=/Import by/i').or(page.locator('h3:has-text("Import Readwise Highlights")')).first()
	...
	test('user can sync Readwise highlights - import 10 items', async ({ page }) => {
		...
		// Use .first() to avoid strict mode violation (multiple elements might match)
		await expect(
			page.locator('text=/Imported|new highlight|Sync completed|already in your inbox|highlights|Syncing|Progress/i').first()
