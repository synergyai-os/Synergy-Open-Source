...
export function buildInputRules(schema: Schema) {
	...
	rules.push(
		new InputRule(/^```$/, (state, match, start, end) => {
			const { tr } = state;
			tr.replaceWith(start, end, schema.nodes.code_block.create({ language: null }));
			return tr;
		})
	);
	...
	rules.push(
		new InputRule(/^```(\w+)\s$/, (state, match, start, end) => {
			...
			const { tr } = state;
			tr.replaceWith(start, end, schema.nodes.code_block.create({ language }));
			return tr;
		})
	);
		...
		props: {
			handlePaste(view, event) {
				...
				if (text && text.length > 100) {
					// Only detect on substantial pastes
...
export function createEditorState(
	...
	return EditorState.create({
		doc: doc ?? noteSchema.nodes.doc.create(),
		plugins
	});
}
