...
| Pre-commit hook conflicts or doesn't run, manual hook installation overwrites Husky hooks        | Use Husky exclusively, update install scripts to skip pre-commit if Husky manages it | [ci-cd.md#L1905](ci-cd.md#L1905)                                    |
...
| Need to identify orphaned tokens (0 usages) before deprecating them           | Analyze token usage: extract tokens, map utilities, scan files, output JSON with file mappings | [ci-cd.md#L2270](ci-cd.md#L2270)                            |
...
| Deprecated tokens still used in codebase, migration not enforced                | Add pre-commit hook: `node scripts/validate-tokens.js --fail-on-deprecated` blocks commits if deprecated tokens used | [ci-cd.md#L2750](ci-cd.md#L2750)                            |
...
| Need to fix semantic token violations - convert hardcoded values to DTCG references | Map values to base tokens, convert exact matches to {spacing.X}, document exceptions for non-matching values | [ci-cd.md#L2600](ci-cd.md#L2600)                            |
...
|| SVG component sizing doesn't work with Recipe system (CSS classes don't apply width/height)      | Use manual style approach for SVG, recipes for CSS components only            | [recipe-system.md#L70](recipe-system.md#L70)                        |
